...
###     get intersection with all cycle edges
    endpoints = []
    for target_cycle_idx in uniq_cycle_idx:#tqdm.tqdm , desc='Iterating regions')
...
def get_partitions_with_db(
    ...
    n_lay = len(NN.layers)-1 if all_net else n_lay
    for current_layer in range(1,n_lay):
        ...
        with torch.no_grad():
            ...
            Wb =  NN.layers[current_layer].get_weights(dtype=torch.float32).cpu()
            ...
            start = 0
            for in_batch in dloader:#tqdm.tqdm,desc='Get Abw',total=len(dloader))
