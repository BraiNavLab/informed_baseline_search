...
class model_wrapper(object):
    ...
    def __init__(self, model, input_shape=None, is_classifier=False,
                 ...
                 device='cpu',as_sequential=True,T=None, dtype=torch.float32):
    ...
    def compose_layers(self,layers,layer_names):
        ...
        for layer,name in zip(layers,layer_names):
            ...
            if type(layer) == torch.nn.modules.linear.Linear:
                new_layers.append(linear(layer,device=self.device,dtype=self.dtype, is_classifier=self.is_classifier))
