...
#     hyp_endpoints = v.reshape(-1,2,v.shape[-1]) ## 
    ...
    for ii,each_hyp in enumerate(uniq_hyp_idx):#tqdm.tqdm(enumerate(uniq_hyp_idx), desc='iterating hyps', total=len(uniq_hyp_idx)):
...
@torch.no_grad()
def to_next_layer_partition(cycles, Abw, current_layer, NN, dtype=torch.float64, device='cpu'):
    ...
    for target_cycle_idx in uniq_cycle_idx:#tqdm.tqdm(uniq_cycle_idx):
...
def _batched_gpu_op_2(method, data1, data2, batch_size, out_size, dtype=torch.float32, workers=2):
    ...
    for in_batch1,in_batch2 in dataloadr:
        ...
        out_batch = method(in_batch1.cpu(),in_batch2.cpu())
...
#     hyp_endpoints = v.reshape(-1,2,v.shape[-1])
    ...
    ## for each intersected cycle, find new regions
    for target_cycle_idx in uniq_cycle_idx:#tqdm.tqdm(uniq_cycle_idx, desc='Iterating regions'):
