...
class DeepView:
	...
	def __init__(self, pred_fn, classes, max_samples, batch_size, data_shape, n=5, 
				 lam=0.65, resolution=100, cmap='tab10', interactive=False, verbose=True,
	...
	def distances_bs(self):
		...
		to emphasise prediction properties (lambda < 0.5).
		'''
		eucl_scale = 1. / self.eucl_distances_bs.max()
		fisher_scale = 1. / self.discr_distances_bs.max()
		eucl = self.eucl_distances_bs * eucl_scale * self.lam
		fisher = self.discr_distances_bs * fisher_scale * (1.-self.lam)
		stacked = np.dstack((fisher, eucl))
		return stacked.sum(-1)
	...
	def _init_plots(self, fig=None, ax=None):
		'''
		Initialises matplotlib artists and plots.
		'''
		if self.interactive:
			plt.ion()
		self.fig, self.ax = plt.subplots(1, 1, figsize=(8, 8)) if  (fig is None and ax is None) else fig, ax
		self.ax.set_title(self.title)
		# self.desc = self.fig.text(0.5, 0.02, '', fontsize=8, ha='center')
		self.cls_plot = self.ax.imshow(np.zeros([10, 10, 3]), aspect="auto", 
			interpolation='gaussian', zorder=0, vmin=0, vmax=1)

		self.sample_plots = []

		for c in range(self.n_classes):
			# color = self.cmap(c/(self.n_classes-1)+1)
			color = self.cmap(c+1)
			plot = self.ax.plot([], [], 'o', label=f"Class {self.classes[c]}", 
	...
	def update_mappings_baseline(self):
		if self.verbose:
			print('Embedding bs samples ...')
		
		# self.mapper.fit(self.distances)
		self.embedded_bs = self.mapper.transform(self.distances_bs)
		# self.inverse.fit(self.embedded_bs, self.samples)
		# self.classifier_view = self.compute_grid()
		# self.background_at = np.array([self.get_mesh_prediction_at(x, y) for x, y in self.embedded_bs])
  
	def inverse_mapping_bs(self):

		return self.inverse(self.embedded_bs)
	...
	def add_samples_baseline(self, samples, labels):
		'''
		Adds samples points to the visualization.
		...
		# calculate new distances
		new_eucl, new_discr = calculate_fisher(self.model, samples, self.samples, 
			self.n, self.batch_size, self.n_classes, self.metric, self.disc_dist, self.verbose)
		# add new distances
		self.discr_distances_bs = self.update_matrix(self.discr_distances_bs, new_discr)
		self.eucl_distances_bs = self.update_matrix(self.eucl_distances_bs, new_eucl)

		# update mappingss
		self.update_mappings_baseline()
	...
	def compute_grid(self):
		...
		color = self.cmap(((self.mesh_classes/mesh_max_class)+1).astype(np.int8))
		...
		h = np.clip(h*1.5, 0, 1)
		color = color[:,0:3]
		color = (1-h)*(0.8*color) + h*np.ones(color.shape, dtype=np.uint8)
	...
	def show(self, baseline = False,bs_color = 'g', last = 10, alpha_bs = 1, data_to_plot= None, fig=None, ax=None):
		'''
		Shows the current plot.
		'''
		# if not (fig is None and ax is None):
		# 	self.fig = fig
		# 	self.ax = ax
		# if not hasattr(self, 'fig'):
		self._init_plots(fig,ax)
		...
		bs_data = self.embedded[last:]
		embedded = self.embedded[:last]
		y_pred = self.y_pred[:last]
		# self.y_true = self.y_true[:last]
		# self.background_at =self.background_at[:last]

		for c in range(self.n_classes):
			data = embedded[y_pred==c]
		...
		if baseline:
			self.ax.scatter(*bs_data.transpose(),color = bs_color, zorder=100000, label="Bs", alpha=alpha_bs)

		if not (data_to_plot is None):
			self.ax.scatter(*data_to_plot.T,color = 'k', zorder=100000)
